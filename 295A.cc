...
struct STree2 { // example: range sum with range addition
	vector<int> st,lazy;int n;
	STree2(int n): st(4*n+5,0), lazy(4*n+5,0), n(n) {}
	void init(int k, int s, int e, int *a){
		lazy[k]=0;  // lazy neutral element
		if(s+1==e){st[k]=a[s];return;}
		int m=(s+e)/2;
...
integer main(){
	int n,m,k; cin>>n>>m>>k;
	fore(i,0,n)cin>>a[i];
	pair<pair<int,int>,int> op[m];
	fore(i,0,m){
		int l,r,d; cin>>l>>r>>d; l--;r--;
		op[i]=mp(mp(l,r),d);
	}
	STree times(m+1); times.init(b);
	fore(i,0,k){
		int x,y; cin>>x>>y; x--; y--;
		times.upd(x,y+1,1);
	}
	fore(i,0,m)op[i].sd*=times.query(i,i+1);
	/*fore(i,0,m){
		cout<<op[i].fs.fs<<" "<<op[i].fs.sd<<" "<<op[i].sd<<endl;
	}*/
	STree2 st(n+1); st.init(a);
	fore(i,0,m){
		st.upd(op[i].fs.fs,op[i].fs.sd+1,op[i].sd);
	}
